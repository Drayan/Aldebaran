    ...
    vulkan_image* out_image)
{
    ...
    VK_CHECK(
        vkCreateImage(context->device.logical_device, &image_create_info, context->allocator, &out_image->handle));
    ...
    if (memory_type == -1)
    {
        AERROR("Required memory type not found. Image not valid.");
    }
    ...
    VkImageAspectFlags aspect_flags)
{
    ...
    VK_CHECK(
        vkCreateImageView(context->device.logical_device, &view_create_info, context->allocator, &image->view));
}
