...
b8 vulkan_device_create(vulkan_context* context)
{
    ...
    if (!compute_shares_graphics_queue)
    {
        indices[index++] = context->device.compute_queue_index;
    }
    ...
    for (u32 i = 0; i < index_count; ++i)
    {
        ...
        // {
        //     // TODO: Check for the max number of queues availables.
    ...
    const char* extension_names[2] = {VK_KHR_SWAPCHAIN_EXTENSION_NAME, "VK_KHR_portability_subset"};
...
void vulkan_device_destroy(vulkan_context* context)
{
    ...
    AINFO("Destroying logical device...");
    ...
    AINFO("Releasing physical device resources...");
...
void vulkan_device_query_swapchain_support(
    ...
    vulkan_swapchain_support_info* out_swapchain_info)
{
    // Surface capacilities
    VK_CHECK(
        vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physical_device, surface, &out_swapchain_info->capabilities));
    ...
    if (out_swapchain_info->format_count != 0)
    {
        if (!out_swapchain_info->formats)
        {
            out_swapchain_info->formats
                = aallocate(sizeof(VkSurfaceFormatKHR) * out_swapchain_info->format_count, MEMORY_TAG_RENDERER);
        }
    ...
    if (out_swapchain_info->present_mode_count != 0)
    {
        ...
        {
            out_swapchain_info->present_modes = aallocate(
                sizeof(VkPresentModeKHR) * out_swapchain_info->present_mode_count,
                MEMORY_TAG_RENDERER);
        }
...
b8 select_physical_device(vulkan_context* context)
{
    ...
    {
        AFATAL("No devices which support Vulkan where found.");
        return FALSE;
    }
    ...
    for (u32 i = 0; i < physical_device_count; ++i)
    {
        ...
        // TODO: There requirements should probably be driven by engine config.
        ...
        if (result)
        {
            AINFO("Selected device: '%s'.", properties.deviceName);
            switch (properties.deviceType)
            {
            default:
            case VK_PHYSICAL_DEVICE_TYPE_OTHER:
                AINFO("GPU type is Unknown.");
                break;

            case VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU:
                AINFO("GPU type is Integrated.");
                break;

            case VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU:
                AINFO("GPU type is Discrete.");
                break;

            case VK_PHYSICAL_DEVICE_TYPE_CPU:
                AINFO("GPU type is CPU.");
                break;
            }

            AINFO(
                "GPU Driver version: %d.%d.%d.%d",
            ...
            for (u32 j = 0; j < memory.memoryHeapCount; ++j)
            {
                ...
                {
                    AINFO("Local GPU memory: %.2f GiB", memory_size_gib);
                }
                else
                {
                    AINFO("Shared System memory: %.2f GiB", memory_size_gib);
                }
            }
    ...
    {
        AERROR("No physical device were found which meet the requirements.");
        return FALSE;
    }

    AINFO("Physical device selected.");
    return TRUE;
}
...
b8 physical_device_meets_requirements(
    ...
    vulkan_swapchain_support_info* out_swapchain_info)
{
    // Evaluate dfevice properties to determine if it meets the needs of our application.
    ...
    if (requirements->discrete_gpu)
    {
        ...
        {
            AINFO("Device is not a discrete GPU, which is required. Skipping.");
            return FALSE;
        }
    }
    ...
    AINFO("Graphics | Present | Compute | Transfer | Name");
    ...
    for (u32 i = 0; i < queue_family_count; ++i)
    {
        ...
        {
            // Take the index if it is the current lowest. This inscreases the
    ...
    {
        AINFO("Device meets queue requirements.");
        ATRACE("Graphics Family Index: %i.", out_queue_family_info->graphics_family_index);
        ATRACE("Present Family Index: %i.", out_queue_family_info->present_family_index);
        ATRACE("Compute Family Index: %i.", out_queue_family_info->compute_family_index);
        ATRACE("Present Family Index: %i.", out_queue_family_info->present_family_index);
        ...
        if (out_swapchain_info->format_count < 1 || out_swapchain_info->present_mode_count < 1)
        {
            ...
            AINFO("Required swapchain support not present, skipping device.");
            return FALSE;
        }
        ...
        if (requirements->device_extension_names)
        {
            ...
            if (available_extension_count != 0)
            {
                available_extensions
                    = aallocate(sizeof(VkExtensionProperties) * available_extension_count, MEMORY_TAG_RENDERER);
        ...
        {
            AINFO("Device does not support samplerAnisotropy, skipping.");
            return FALSE;
        }
