    ...
    u32 stencil)
{
    ...
    u32 attachement_description_count = 2;
    VkAttachmentDescription attachement_descriptions[attachement_description_count];

    // Color attachment
    VkAttachmentDescription color_attachement;
    color_attachement.format = context->swapchain.image_format.format; // TODO: Make this configurable.
    ...
    color_attachement.initialLayout
        = VK_IMAGE_LAYOUT_UNDEFINED; // Do not expect any particular layout before render pass starts.
    color_attachement.finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR; // Transitioned to after the render pass.
    color_attachement.flags = 0;

    attachement_descriptions[0] = color_attachement;

    VkAttachmentReference color_attachement_reference;
    color_attachement_reference.attachment = 0; // Attachment description array index
    ...
    attachement_descriptions[1] = depth_attachment;
    ...
    render_pass_create_info.attachmentCount = attachement_description_count;
    render_pass_create_info.pAttachments = attachement_descriptions;
    ...
    VkFramebuffer frame_buffer)
{
    ...
    azero_memory(clear_values, sizeof(VkClearValue) * 2);
