    ...
    u32* out_image_index)
{
    ...
    {
        AFATAL("Failed to acquire swapchain image!");
        return FALSE;
    }
    ...
    u32 present_image_index)
{
    ...
    {
        AFATAL("Failed to present swapp chain image!");
    }
}
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain)
{
    ...
    swapchain_extent.width = ACLAMP(swapchain_extent.width, min.width, max.width);
    swapchain_extent.height = ACLAMP(swapchain_extent.height, min.height, max.height);
    ...
    if (context->device.graphics_queue_index != context->device.present_queue_index)
    {
        u32 queueFamilyIndices[]
            = {(u32)context->device.graphics_queue_index, (u32)context->device.present_queue_index};
    ...
    VK_CHECK(
        vkGetSwapchainImagesKHR(context->device.logical_device, swapchain->handle, &swapchain->image_count, 0));
    if (!swapchain->images)
    {
        swapchain->images = (VkImage*)aallocate(sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    if (!swapchain->views)
    {
        swapchain->views
            = (VkImageView*)aallocate(sizeof(VkImageView) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    ...
    if (!vulkan_device_detect_depth_format(&context->device))
    {
        ...
        AFATAL("Failed to find a supported format!");
    }
    ...
    AINFO("Swapchain created successfully.");
}
...
void destroy(vulkan_context* context, vulkan_swapchain* swapchain)
{
    ...
    // Only destroy the view, not the images, since those are owned by the swapchain and are thus
